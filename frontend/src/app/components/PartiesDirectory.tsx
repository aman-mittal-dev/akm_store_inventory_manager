      ...
      <CardContent className="p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xl font-bold text-foreground">
            {isCurrency ? formatINR(amount) : amount}
          </p>
        </div>
      </CardContent>
    </Card>
  ...
  const filteredCustomers = useMemo(
    () =>
      customers.filter(customer => {
        const contactDigits = normaliseContact(customer.contact);

        return (
          customer.name.toLowerCase().includes(searchTerm) ||
          (searchDigits && contactDigits.includes(searchDigits))
        );
      }),
    [customers, search],
  );
  ...
  const filteredSuppliers = useMemo(
    () =>
      suppliers.filter(supplier => {
        const contactDigits = normaliseContact(supplier.contact);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <SummaryCard label="Customers" amount={customers.length} icon={Users} colorClass="bg-purple-100 text-purple-600" isCurrency={false} />
        <SummaryCard label="Suppliers" amount={suppliers.length} icon={Building2} colorClass="bg-amber-100 text-amber-600" isCurrency={false} />
      </div>
            ...
            <>
              ...
              {filteredCustomers.map(c => (
                <PartyRow
                  ...
                  billPath={id => `/bill/${id}`}
                // billPath={id => `/transactions/incoming/${id}`}
                />
              ))}
            </>
          )}
          ...
          {filteredSuppliers.length === 0 ? (
            ...
            <>
              ...
              {filteredSuppliers.map(s => (
                ...
                // billPath={id => `/transactions/outgoing/${id}`}
                />
              ))}
            </>
          )}
